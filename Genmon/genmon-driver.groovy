...
// Clears every stored attribute value, then immediately re-polls.
//
// Genmon decides what it sends, via its own "Excluded Data Paths" blacklist
// and the Include Monitor Stats / Include Weather toggles. When one of those
// is changed to drop a field, the driver simply stops receiving it — there is
// no "removed" message — so the attribute lingers on the device forever at
// whatever value it last held, which is misleading on a dashboard and in
// rules. Clearing and re-polling lets the next update repopulate only what
// genmon is actually still sending; anything it has stopped sending stays
// gone.
//
// connectionStatus and healthStatus are generated by the driver rather than
// sourced from genmon, so they are preserved — wiping healthStatus would
// leave the device looking unknown until the next scheduled check.
def resetAttributes() {
    def preserve = ["connectionStatus", "healthStatus"]
    def removed  = []

    device.currentStates.each { st ->
        if (!(st.name in preserve)) {
            device.deleteCurrentState(st.name)
            removed << st.name
        }
    }

    if (removed) {
        log.info "Genmon: reset ${removed.size()} attribute(s) — ${removed.sort().join(', ')}"
    } else {
        log.info "Genmon: reset attributes — nothing stored to clear"
    }
    log.info "Genmon: re-polling; whatever genmon still sends will repopulate"
