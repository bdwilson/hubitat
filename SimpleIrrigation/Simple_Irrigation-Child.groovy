    ...
    dynamicPage(name: "", title: "", install: true, uninstall: true, refreshInterval:0) {
		...
		section(getFormat("header-green", "${getImage("Blank")}"+" Notification Options")) {
            ...
            if(sendPushMessage) {
                ...
                input "sendSafetyPushMessage", "bool", title: "Send open notification even if weather switch has cancelled the schedule (will only happen if Other Switch is off).", defaultValue:false, required:true
            }
		}
    ...
    if(state.daysMatch) {
        if(state.canWater) {
            ...
            if(state.valveStatus == "closed" || state.switchStatus == "off") {
                ...
                if(valveORswitch) {
                    if(logEnable) log.debug "In turnValveOn for ${onLength}- trying to turn on - Attempt ${state.valveTry} - will check again in 20 seconds"
                    switchDevice.on()
                } else {
            ...
            if (!state.canWater) {
                state.msg = "${valveDevice} is now ${state.valveStatus}. Watering session skipped due to weather/wind switch."
            }
            if ((!state.canWater && sendSafetyPushMessage == true && state.otherDevice != "on")) { // state.otherDevice is master disable; don't alert when master disable is on.
                if(sendInfoPushMessage) pushHandler()
            }
            resetTrys()
            turnValveOff()
