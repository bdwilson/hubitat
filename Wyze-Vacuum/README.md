...
**"Will resume" exits don't count as finished, fixed in 1.24.0.** The vacuum's own `mode` signal distinguishes two different reasons for leaving `Cleaning` to return to the dock: mode `10` ("cleaning completed, returning to charge") is a genuine finish, but mode `11` ("docked, cleaning will resume") means the vacuum only paused itself — usually for a battery top-up — and fully intends to pick the same job back up on its own. This was previously collapsed into the same "Returning to charge" status as a real finish, so a room could get credited (and the run cleared) the moment mode 11 appeared, even though the vacuum then resumed and kept cleaning for a while longer, uncredited and untracked. Confirmed live: a Kitchen run was credited at 9 minutes when mode 11 appeared, then the vacuum resumed 1h42m later and cleaned for another 25 minutes on its own — 34 real minutes total, only 9 of them ever recorded. Now a mode-11 exit leaves the run active and carries its elapsed time forward instead of crediting/clearing it, so the eventual genuine finish (mode 10, or any other real exit) gets credited with the full total across both segments. If the vacuum never actually resumes, a 3-hour safety timeout force-finishes the run with whatever was measured, so a room can't get stuck "active" forever.
...
These three (`roomsPendingThisCycle`, `nextRoomsToClean`, `nextRoomDueAt`), plus `lastRefresh`, get recomputed on every poll, but only actually create a new device event when the value changed since the last poll — not a fresh identical entry every single cycle, which would otherwise fill up the device's event history with noise for values that usually don't move minute to minute. **A real bug here, fixed in 1.25.1:** each poll cycle actually runs two independent async HTTP calls (props and status), and both of their callbacks used to update these same four attributes — so instead of one entry per poll, you'd see two near-identical ones milliseconds apart. The dedup logic itself wasn't broken (values genuinely didn't change poll-to-poll), but the second callback's read of the device's current value doesn't reliably see the first callback's write yet, since they're two separate concurrent async contexts — a timing gap the dedup check couldn't see across. These attributes are now only updated from one of the two callbacks, so each poll produces at most one entry per attribute, same as intended in 1.22.0.
...
Added in 1.24.0. A healthy vacuum is always doing one of: cleaning, returning to charge, or actually charging. If a poll reports `status: Standby` (or otherwise idle) while `charge_state` is **not** charging, and that persists for 30+ minutes straight, that's not a normal state to sit in — it's treated as a likely physical stuck condition (wedged under furniture, wheel caught, etc.) and you get one push notification for that episode. It won't repeat every poll while still stuck, and it resets cleanly (so a future episode notifies again) once the vacuum is next seen cleaning or charging.
